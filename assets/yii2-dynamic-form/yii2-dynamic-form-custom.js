...
(function ($) {
    ...
    var _restoreSpecialJs = function(widgetOptions) {
        ...
        var $hasAutoNumeric =  $(widgetOptionsRoot.widgetItem).find('[id$=-disp]');
        if ($hasAutoNumeric.length > 0) {
            $hasAutoNumeric.each(function() {
                //Define fields to clone
                configFields = ['aSep', 
                                'dGroup',
                                'aDec',
                                'altDec',
                ...
                //console.log("this: " + id);
                //console.log($(this));
                //console.log($(this).autoNumeric('getSettings'));
                
                
                var settings = $(this).autoNumeric('getSettings');
                
                if (typeof settings !== 'undefined') {
                    configFields.forEach(function(configKey) {
                            autoNumericConfig[configKey]= settings[configKey];
                    });
                    //return;
                }
                
                
                //search for a id which begins with the same string as the -disp field, but without the -disp value
                //Example 'id="input-0-disp" => Search for id="input-0"
                var $hasBaseField = $(widgetOptionsRoot.widgetItem).find('#' + $(this).attr('id').replace('-disp',''));
                if ($hasBaseField.length >0) {
                    
                    //console.log("Base: " + $hasBaseField.attr('id'));
                    //console.log($hasBaseField);
                    
                    var val = parseFloat($hasBaseField.val());
                    //console.log("config:");
                    //console.log(autoNumericConfig);
                    $(this).autoNumeric('init', autoNumericConfig);
                    $(this).autoNumeric('set', val);
                    $(this).on('change', function () {
                        var unformatted = $(this).autoNumeric('get');
                        $hasBaseField.val(unformatted);
                        $hasBaseField.trigger('change');
                    });                    
                }
